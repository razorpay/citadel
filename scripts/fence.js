...
module.exports = function(md) {
  ...
  return function(state, startLine, endLine, silent) {
    ...
    if (sCountStart - state.blkIndent >= 4) { return false; }
    ...
    token.content = makeFenceContent(state.getLines(startLine, nextLine, len, true));
    token.markup  = markup;
  ...
  function makeFenceContent(content) {
    ...
    var content = content.split(/^[`-]{3}/mg)
      ...
      .reduce((html, code, index)=> {
        ...
        if (content[cursor] === '-') {
          ...
          if (info && info[2]) {
            ...
            if(languages.findIndex(l => l['lang'] === lang) === -1){
              languages.push({
                lang,
                title
              });
          ...
          if (title) {
            html += `<input data-lang="${lang}" ${index ? '': 'checked '} id="${labelFor}" name="radio-${radioCount}" type="radio">`;
