...
module.exports = function (md) {
  md.renderer.rules.html_block = function (tokens, idx, options, env, self) {
    ...
    if (rawText) {
      ...
      return processSingleLine(lines[0], md);
    } else {
      ...
      var innerHTML = doprocess(tag, contentLines, md);
      var endTag = `</${tag}>`;
      if (tag === 'table') {
        return (
  ...
  return function (state, startLine, endLine, silent) {
    ...
    (token.tag = tagMatch[1]), (token.lines = lines);
      ...
      lines
        ...
        .reduce((prev, next) => {
          ...
          if (delim_row) {
            ...
            prev += next
              .split('|')
              .map((col) => `<${td}>${md.render(col.trim())}</${td}>`)
              .join('');
          }
          return prev;
        }, '') +
...
function doprocess(tag, lines, md) {
...
function processSingleLine(line, md) {
  var html = '';
  var currenTag;
  var Parser = new htmlparser.Parser({
    ...
    ontext: function (text) {
      html += doprocess(currenTag, [text], md);
    },
